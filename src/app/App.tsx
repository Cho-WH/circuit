...
import { buildPotentialModel, defaultPotentialPalette, makePath, suggestPaths, type PotentialPaletteId } from '../visualization';
...
export function App() {
  ...
  const [potentialPalette, setPotentialPalette] = useState<PotentialPaletteId>(defaultPotentialPalette);
  ...
  const potential = useMemo(() => buildPotentialModel(doc, compilation.circuit, result, { scale: heightScale, palette: potentialPalette, ...(fixedRange ? { range: { min: rangeMin, max: rangeMax } } : {}) }), [doc, compilation, result, heightScale, potentialPalette, fixedRange, rangeMin, rangeMax]);
