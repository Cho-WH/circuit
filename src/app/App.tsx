...
export function App() {
  ...
  return <div className={`app-shell mode-${mode}${mode === 'potential' && potentialView === '3d' ? ' view-3d' : ''}${presentation ? ' presentation' : ''}`}>
    ...
    <nav className="modebar" aria-label="작업 모드"><div className="mode-tabs"><button className={mode === 'build' ? 'active' : ''} onClick={() => { setMode('build'); cancelTool(); }}><MousePointer2 size={16}/>회로 만들기</button><button className={mode === 'potential' ? 'active' : ''} onClick={() => { setMode('potential'); cancelTool(); }}><Zap size={16}/>전위 보기</button><button className={mode === 'measure' ? 'active' : ''} onClick={() => { setMode('measure'); cancelTool(); setSelected([]); setHovered(null); setRoutePreview(false); }}><Crosshair size={16}/>측정하기</button><button className={mode === 'worksheet' ? 'active' : ''} onClick={() => { setMode('worksheet'); cancelTool(); }}><Copy size={16}/>문제 만들기</button></div><button className="presentation-toggle" onClick={()=>setPresentation(v=>!v)}>{presentation?'편집 화면':'수업 화면'}</button><div className="mode-meta"><span className="pill">DC</span>이상적인 직류 회로<span className="separator"/>MVP 0.1 · 문서 v1</div></nav>
    <div className="workspace">
      <aside className="library-panel" hidden={mode === 'measure'}><div className="section-heading"><h2>부품 라이브러리</h2><span>6</span></div><p className="muted">선택한 뒤 캔버스에 놓으세요</p>
      ...
      <main className="canvas-column"><div className="editor-toolbar" hidden={mode === 'measure'}><div className="tool-group">{[{ id: 'select', label: '선택', Icon: MousePointer2 }, { id: 'wire', label: '배선', Icon: Cable }, { id: 'junction', label: '분기점', Icon: GitBranch }, { id: 'pan', label: '화면 이동', Icon: Hand }, { id: 'reference', label: '기준점', Icon: Crosshair }].map(({ id, label, Icon }) => <button key={id} className={tool === id && !placement ? 'active' : ''} title={label} aria-label={label} onClick={() => { setTool(id); setPlacement(null); setWireStart(null); }}><Icon size={18}/><span>{label}</span></button>)}</div><span className="toolbar-separator"/><div className="tool-group"><button aria-label="실행 취소" title="실행 취소 Ctrl+Z" disabled={!history.past.length} onClick={() => setHistory(undo(history))}><Undo2 size={18}/></button><button aria-label="다시 실행" title="다시 실행 Ctrl+Shift+Z" disabled={!history.future.length} onClick={() => setHistory(redo(history))}><Redo2 size={18}/></button></div><button className="tidy-button" onClick={() => setRoutePreview(v => !v)}>배선 정리</button></div>
        ...
        <div className="hint-strip" hidden={mode === 'measure'}><span>{placement ? `${componentDefinitions[placement].name}를 놓을 위치를 클릭하세요. Esc로 취소합니다.` : tool === 'wire' ? wireStart ? '연결할 두 번째 단자를 선택하세요.' : '시작 단자를 선택하세요. 단자를 직접 눌러도 배선됩니다.' : tool === 'junction' ? '도선 위를 누르면 전기적으로 연결된 분기점이 만들어집니다.' : tool === 'reference' ? '0 V로 삼을 단자 또는 분기점을 선택하세요.' : 'Shift+클릭 복수 선택 · 방향키 이동 · R 회전 · W 배선 · Ctrl+Z 실행 취소'}</span></div>
      </main>
      <aside className="inspector-panel" hidden={mode === 'measure'}><div className="section-heading"><h2><SlidersHorizontal size={16}/>속성</h2>{selected.length > 0 && <button aria-label="선택 해제" onClick={() => setSelected([])}><X size={16}/></button>}</div>
