...
import { Zap, MousePointer2, Cable, Hand, RotateCw, Trash2, Undo2, Redo2, Copy, ClipboardPaste, Plus, Download, FolderOpen, Save, X, CircleHelp, Check, AlignHorizontalJustifyCenter, Crosshair, ChevronRight, SlidersHorizontal, CircuitBoard, MessageCircle } from 'lucide-react';
import { cloneDocument, emptyDocument, type CircuitDocument, type ComponentType, type EndpointRef, type Point } from '../domain';
import { componentDefinitions, componentValue, createComponent, symbolMarkup, formatQuantity, endpointName, type WorksheetMode, type NumberFormat } from '../component-library';
import { createHistory, executeCommand, executeCommands, undo, redo, copySelection, parseValue, type Command, type PastePayload } from '../editor';
...
export function App() {
  ...
  function cancelTool() { setPlacement(null); setTool('select'); setWiringResetKey(key=>key+1); }
  useEffect(() => {
  ...
  return <div className={`app-shell mode-${mode}${mode === 'potential' && potentialView === '3d' ? ' view-3d' : ''}${detailsOpen ? ' details-open' : ''}${presentation ? ' presentation' : ''}`}>
    ...
    <div className="workspace">
      <aside className="library-panel" hidden={mode !== 'build'}><div className="section-heading"><h2>부품 라이브러리</h2><span>6</span></div><p className="muted">선택한 뒤 캔버스에 놓으세요</p>
        ...
        <label className="field-label">예제 회로<select aria-label="예제 회로" value={examples.some(x => x.document.documentId === doc.documentId) ? doc.documentId : ''} onChange={e => { const ex = examples.find(x => x.document.documentId === e.target.value); if (ex) replace(layoutExample(ex.document)); }}><option value="" disabled>예제 선택</option>{[['탐구 예제',examples.filter(e=>!['fix-06','fix-07','fix-08'].includes(e.document.documentId)&&e.id!=='UX-WIRE')],['연결 연습',examples.filter(e=>e.id==='UX-WIRE')],['진단 연습',examples.filter(e=>['fix-06','fix-07','fix-08'].includes(e.document.documentId))]].map(([label,items])=><optgroup key={label as string} label={label as string}>{(items as typeof examples).map(e=><option key={e.id} value={e.document.documentId}>{e.title}</option>)}</optgroup>)}</select></label>
      ...
      <aside className="inspector-panel" hidden={mode === 'measure'||!detailsOpen}><div className="section-heading"><h2><SlidersHorizontal size={16}/>속성</h2>{selected.length > 0 && <button aria-label="선택 해제" onClick={() => setSelected([])}><X size={16}/></button>}</div>
        <div hidden={mode !== 'worksheet'}><WorksheetPanel active={mode==='worksheet'} document={doc} result={result} selected={selected} mode={worksheetMode} onMode={setWorksheetMode} numberFormat={numberFormat} onNumberFormat={setNumberFormat} anchor={annotationAnchor} onAnchor={setAnnotationAnchor} onSelect={id=>setSelected([id])} dispatch={dispatch} newId={newId} onNotice={setNotice}/></div>

        {mode === 'potential' && <div className="potential-settings"><h3>같은 연결, 같은 전위</h3><p className="tiny-note">단자나 도선을 누르면 같은 절점 전체가 강조됩니다.</p>{selectedNet&&potential.nets[selectedNet]&&<div className="net-readout"><strong>{formatQuantity(potential.nets[selectedNet].voltage,'V')}</strong><p>{potential.nets[selectedNet].endpointIds.map(id=>endpointName(doc,id)).join(' · ')}</p></div>}<label className="field-label">색상 범위<select value={fixedRange?'fixed':'auto'} onChange={e=>setFixedRange(e.target.value==='fixed')}><option value="auto">자동 범위</option><option value="fixed">고정 범위로 비교</option></select></label>{fixedRange&&<div className="range-inputs"><label>최소 V<input aria-label="고정 범위 최소" type="number" value={rangeMin} onChange={e=>setRangeMin(e.target.valueAsNumber)}/></label><label>최대 V<input aria-label="고정 범위 최대" type="number" value={rangeMax} onChange={e=>setRangeMax(e.target.valueAsNumber)}/></label></div>}{fixedRange&&(!Number.isFinite(rangeMin)||!Number.isFinite(rangeMax)||rangeMin>=rangeMax)&&<p className="tiny-note">최소보다 큰 최대값을 입력하세요. 현재 자동 범위가 적용됩니다.</p>}<label hidden={potentialView!=='3d'} className="field-label">높이 강조 · {(heightScale/18).toFixed(1)}배<input aria-label="높이 강조 배율" type="range" min="4" max="40" value={heightScale} onChange={e=>setHeightScale(Number(e.target.value))}/></label><p className="tiny-note">기본 높이를 1배로 삼습니다. 높이는 실제 공간 높이가 아닌 전위의 표현이며 전압값은 변하지 않습니다. 화살표는 관습적 전류 방향입니다. 화살표 굵기는 전류 크기의 상대적 표현이며 전자 속도가 아닙니다. 열린 스위치는 연결하지 않습니다.</p>{potential.undefinedCount>0&&<p className="tiny-note">전위 미정 {potential.undefinedCount}개 절점: 회색으로 표시하고 3D 높이를 부여하지 않습니다.</p>}<div className="library-divider"/></div>}
