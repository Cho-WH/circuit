...
import { annotationPlacements, componentPresentation, componentDefinitions } from '../component-library';
...
export function WorksheetPanel(props: Props) {
  ...
  useEffect(()=>{if(!copied)return;const timer=window.setTimeout(()=>setCopied(false),2400);return()=>window.clearTimeout(timer);},[copied]);
  ...
  async function output(action:'svg'|'png'|'copy'|'preview') {
    ...
    try {
      ...
      else if(action==='copy'){const copied=await copyPng(doc,options,props.result);if(copied.copied)setCopied(true);else{saveBlob(copied.blob,name+'.png');props.onNotice('그림 복사를 지원하지 않아 PNG로 저장했습니다.');}}
