...
export function MeasurementPanel(props: Props) {
  ...
  return <section className="measurement-panel" aria-label="측정 작업 공간">
    ...
    <p className="record-retention">기록은 CSV로 보관하세요. 회로 자동 저장·JSON에는 포함되지 않으며 새로고침·종료하면 사라집니다.</p><details className="measurement-details" open><summary>측정 기록 ({records.length})</summary><div className="record-scroll">{records.length===0&&<div className="records-empty"><Plus size={18}/><span>측정값을 기록하면 여기에 쌓입니다</span></div>}<table hidden={records.length===0}><thead><tr><th>조건</th><th>값</th></tr></thead><tbody>{records.map((r,i)=><tr key={i}><td><Notation text={r.condition}/><small>{r.recordedAt}</small></td><td>{formatQuantity(r.value??undefined,r.unit)}</td></tr>)}</tbody></table></div><button className="wide-button" disabled={!records.length} onClick={()=>{const csv=measurementsToCsv(records);if(csv.ok)download('\uFEFF'+csv.value,'회로-측정기록.csv','text/csv;charset=utf-8');else setMessage('CSV를 만들 수 없습니다.');}}>CSV 저장 · 단위와 회로 조건 포함</button><button disabled={!records.length} onClick={()=>setRecords([])}>기록 비우기</button></details>
