...
import { formatQuantity, endpointName } from '../component-library';
...
function Conservation({ value, unit, document:doc }: { value: ConservationResult; unit: string; document:CircuitDocument }) {
  return <div className="conservation"><p>{value.defined ? `합 ${formatQuantity(value.sum, unit)}${value.passes ? ' · 0과 일치' : ''}` : '측정할 수 없습니다'}</p>{value.defined && <><ul>{value.terms.map((t, i) => <li key={i}><Notation symbol text={doc.components.find(c=>c.id===t.elementId)?.label??t.elementId}/>: {t.value >= 0 ? '+' : ''}{formatQuantity(t.value, unit)}</li>)}</ul><small>허용오차 {formatQuantity(value.tolerance, unit)}</small></>}<Diagnostics items={value.diagnostics}/></div>;
}
function SweepGraph({ data }: { data: ParameterSweepResult }) {
  ...
  return <><svg className="sweep-graph" viewBox="0 0 300 212" role="img" aria-label={`${data.xLabel} (${data.xUnit})에 따른 ${data.yLabel} (${data.yUnit})`}><path d="M52 32V160H282" fill="none" stroke="#8998aa"/><text x="12" y="20" fontSize="12">{data.yLabel} ({data.yUnit})</text><text x="52" y="178" fontSize="11">{formatQuantity(minX,data.xUnit)}</text><text x="275" y="178" fontSize="11" textAnchor="end">{formatQuantity(maxX,data.xUnit)}</text><text x="160" y="204" fontSize="12" textAnchor="middle">{data.xLabel} ({data.xUnit})</text><text x="48" y="40" fontSize="10" textAnchor="end">{formatQuantity(maxY,data.yUnit)}</text><text x="48" y="160" fontSize="10" textAnchor="end">{formatQuantity(minY,data.yUnit)}</text><path d={path} fill="none" stroke="#3478f6" strokeWidth="2"/>{valid.map((s,i)=><circle key={i} cx={x(s.x)} cy={y(s.y!)} r="3" fill="#3478f6"><title>{formatQuantity(s.x,data.xUnit)}: {formatQuantity(s.y??undefined,data.yUnit)}</title></circle>)}</svg>{valid.length < data.samples.length && <p className="tiny-note">계산할 수 없는 지점 {data.samples.length-valid.length}곳은 비워 두었어요.</p>}<details><summary>값 보기</summary><table><thead><tr><th>{data.xLabel} ({data.xUnit})</th><th>{data.yLabel} ({data.yUnit})</th></tr></thead><tbody>{data.samples.map((s,i)=><tr key={i}><td>{Number(s.x.toFixed(2))}</td><td>{s.y===null?'—':Number(s.y.toFixed(2))}</td></tr>)}</tbody></table></details></>;
}
...
export function MeasurementPanel(props: Props) {
  ...
  return <section className={`measurement-panel${showNotebook?' notebook-open':''}`} aria-label="측정 작업 공간">
    ...
    <div className="measure-console">
      ...
      <div className={`measure-result${ready?' ready':''}`}><output aria-label="측정값" aria-live="polite">{kind==='resistance'&&resistance?.status==='open'?'∞ Ω':formatQuantity(reading,unit)}</output><button className="record-reading" aria-label="측정값 기록" title="측정값 기록" disabled={!ready} onClick={record}><Plus size={15}/>기록</button></div>
