...
export interface Annotation {
  id: string; kind: 'label' | 'arrow' | 'blank' | 'question' | 'note';
  anchor: EndpointRef; content: string; visibility: 'always' | 'problem' | 'answer' | 'hidden';
}
...
export interface CircuitDocument {
  $schema?: string; format: 'edu-circuit'; version: 1; documentId: string; title: string;
...
export function validateDocument(input: unknown): DocumentValidation {
  ...
  const refs = [...doc.wires.flatMap(w => [w.start, w.end]), ...doc.annotations.map(a => a.anchor), ...(doc.referenceNode ? [doc.referenceNode] : [])];
...
export function emptyDocument(id = 'untitled'): CircuitDocument {
  return { format: 'edu-circuit', version: 1, documentId: id, title: '새 회로', components: [], wires: [], junctions: [], annotations: [], referenceNode: null, activity: null };
}
