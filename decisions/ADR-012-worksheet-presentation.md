...
Annotation.arrow에 shape(straight/corner), length, legLength, rotation, reversed를 저장한다. position은 회전 기준점이며 회전 각도는 도 단위다. 기존 end 좌표는 호환용으로 보존하고 arrow가 없으면 원래 선분의 길이와 각도를 공통 arrowStyle에서 복원한다. v2 스키마를 보관하고 v1→v2→v3 순차 검증·복제 변환을 제공하므로 기존 배치를 반올림하거나 옮기지 않는다. 전기 계산 계약에는 변화가 없다.

component-library의 arrowGeometry가 경로·화살촉을 계산한다. 편집 판정 영역과 SVG·PNG가 같은 기하를 사용하며 꺾인 점까지 출력 경계에 포함한다. 화살촉은 가는 채움형으로 통일하고 선 끝과 모서리는 둥글게 마감한다. 길이 드래그는 놓을 때 한 명령이며 취소 시 복원한다. 회전 입력은 확정 시 한 명령, Escape는 편집 입력 취소다.

기호 변환은 시작 숫자와 관계없이 문자별로 적용한다. 2I는 정자 2와 이탤릭 I로 표시하고 단위는 값 렌더링에서 정자로 유지한다.
