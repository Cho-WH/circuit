...
function runBoundaryFixture(files: SourceFixture): CheckResult {
  ...
  try {
    const temporaryTool = join(temporaryRoot, "tools", "check-boundaries.mjs");
    mkdirSync(dirname(temporaryTool), { recursive: true });
    copyFileSync(boundaryChecker, temporaryTool);
    ...
    const result = spawnSync(process.execPath, [temporaryTool], {
      cwd: temporaryRoot,
      encoding: "utf8",
    });
    return {
...
describe("module boundary checker", () => {
  it("accepts core dependencies through a module public index", () => {
    const result = runBoundaryFixture({
      ...
      "src/connectivity/index.ts": 'import type { Id } from "../domain";\nexport type NetId = Id;\n',
    });
  ...
  it("rejects imports of another module's private file", () => {
    const result = runBoundaryFixture({
      ...
      "src/connectivity/index.ts": 'import { hidden } from "../domain/private";\nexport { hidden };\n',
    });
