...
터치는 부품 탭 후 배치 또는 길게 눌러 끌기를 지원한다. 빈 공간에는 바로 놓고 도선에는 미리보기 후 **삽입**으로 확정한다. 선택 부품은 바로 끌거나 **이동 → 목적지 탭 → 놓기**를 사용한다. 두 손가락은 화면 이동·확대이며 진행 중 편집을 취소한다. 출력 표기는 방향 버튼으로도 조절한다. 실제 휴대전화·OS 키보드 검증은 남아 있으며 [남은 작업과 검증](docs/implementation/follow-up.md)에 범위를 기록했다.
