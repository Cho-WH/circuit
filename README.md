...
자동 저장은 현재 브라우저에 남으며 JSON으로 별도 보관할 수 있다. 측정 기록은 메모리에 있으므로 페이지를 닫기 전에 CSV로 저장한다. 파일 형식은 `edu-circuit` v4이며 이전 v1·v2·v3 파일도 불러온다. ‘한마디’ 게시판은 로컬 미리보기이며 원격 교사 의견 수집 서비스는 연결하지 않았다.
