...
1. **회로 만들기**: 예제를 열거나 부품을 배치하고 단자에서 배선을 시작한다. 선택 도선의 분기 힌트와 교차점 클릭으로 연결을 편집한다.
...
자동 저장은 현재 브라우저에 남으며 JSON으로 별도 보관할 수 있다. 측정 기록은 메모리에 있으므로 페이지를 닫기 전에 CSV로 저장한다. 파일 형식은 `edu-circuit` v1이다. ‘한마디’ 게시판은 로컬 미리보기이며 원격 교사 의견 수집 서비스는 연결하지 않았다.
